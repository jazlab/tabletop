...
def on_adv_packet(
    ...
    if is_private:
        print(
            "Button "
            + bd_addr
            + " is currently private. Hold it down for 7 seconds to make it public."
        )
        return
    ...
    def on_create(channel, error, connection_status):
        if connection_status == fliclib.ConnectionStatus.Ready:
            ...
            client.set_timer(
                30 * 1000, lambda: client.remove_connection_channel(channel)
            )
    ...
    def on_connection_status_changed(
        channel, connection_status, disconnect_reason
    ):
        if connection_status == fliclib.ConnectionStatus.Ready:
            done(bd_addr)
