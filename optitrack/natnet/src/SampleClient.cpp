 ...
 * This program connects to a NatNet server, receives a data stream, and writes
 *that data stream to an ascii file.  The purpose is to illustrate using the
 *NatNetClient class. Usage [optional]: SampleClient [ServerIP] [LocalIP]
 *[OutputFilename] [ServerIP]			IP address of the server (e.g.
 *192.168.0.107) ( defaults to local machine) [OutputFilename]	Name of points
 *file (pts) to write out.  defaults to Client-output.pts
 *********************************************************************/
...
void NATNET_CALLCONV DataHandler(
    sFrameOfMocapData* data, void* pUserData); // receives data from the server
...
// static const ConnectionType kDefaultConnectionType =
...
int main(int argc, char* argv[]) {
  ...
  g_pClient->SetFrameReceivedCallback(
      DataHandler,
      g_pClient); // this function will receive data from the server

  // [Optional] Automatic Motive server discovery
  // If no arguments were specified on the command line, attempt to discover
  ...
  if ((argc == 1) && (strDefaultLocal.empty() && strDefaultMotive.empty())) {
    ...
    printf("Press the number key that corresponds to any discovered server to "
        ...
        if (serverIndex < g_discoveredServers.size()) {
          ...
          if (discoveredServer.serverDescription.bConnectionInfoValid) {
            ...
            snprintf(
                ...
                discoveredServer.serverDescription
                    .ConnectionMulticastAddress[0],
                discoveredServer.serverDescription
                    .ConnectionMulticastAddress[1],
                discoveredServer.serverDescription
                    .ConnectionMulticastAddress[2],
                discoveredServer.serverDescription
                    .ConnectionMulticastAddress[3]);

            g_connectParams.connectionType =
                discoveredServer.serverDescription.ConnectionMulticast
                    ? ConnectionType_Multicast
                    : ConnectionType_Unicast;
            g_connectParams.serverCommandPort =
                discoveredServer.serverCommandPort;
  ...
  if (!gUpdatedDataDescriptions) {
    printf("[SampleClient] ERROR : Unable to retrieve Data Descriptions from "
           "Motive.\n");
  } else {
  ...
  // Main thread loop
  // Data will be delivered in a separate thread to DataHandler() callback
  // functon
...
int ProcessKeyboardInput() {
  ...
  if (_kbhit()) {
    ...
    case 's':
      ...
      gUpdatedDataDescriptions = UpdateDataDescriptions(true);
      if (!gUpdatedDataDescriptions) {
        printf("[SampleClient] ERROR : Unable to retrieve Data Descriptions "
               "from Motive.\n");
      } else {
        gNeedUpdatedDataDescriptions = false;
      }
    ...
    case 'm':
      ...
      if (iResult == ErrorCode_OK)
        printf(
            "[SampleClient] Client connection type changed to Multicast.\n\n");
      else
        printf("[SampleClient] Error changing client connection type to "
               "Multicast.\n\n");
      break;
    ...
    // note: applies to unicast connections only - indicates to Motive to stop
    // sending packets to this client's endpoint
    case 'd':
    ...
    case 'e': {
      // Note : Smoothing value is actually float [0.0,1.0].  Motive UI formats
      // it to [0-100]
      char szCommand[kMaxMessageLength];
      sprintf(szCommand, "SetProperty,RigidBody1,Smoothing,%f",
              gSmoothingValue);
...
/**
 * \brief [optional] called by NatNet with a list of automatically discovered
 * Motive instances on the network(s).
 *
...
void NATNET_CALLCONV ServerDiscoveredCallback(
  ...
  printf("[%c] %s %d.%d.%d at %s ", serverHotkey,
  ...
  if (pDiscoveredServer->serverDescription.bConnectionInfoValid) {
    printf("(%s)", pDiscoveredServer->serverDescription.ConnectionMulticast
                       ? "multicast"
                       : "unicast");
  } else {
    printf("(WARNING: Legacy server, could not auto-detect settings. "
...
int ConnectClient() {
  ...
  if (retCode != ErrorCode_OK) {
    ...
    printf("[SampleClinet] Unable to connect to server.  Error code: %d. "
           "Exiting.\n",
           retCode);
    ...
    if (ret != ErrorCode_OK || !g_serverDescription.HostPresent) {
      printf("[SampleClient] Unable to connect to server. Host not present. "
             "Exiting.\n");
      return 1;
    }
...
void UpdateDataToDescriptionMaps(sDataDescriptions* pDataDefs) {
  ...
  for (int i = 0; i < pDataDefs->nDataDescriptions; i++) {
    ...
    if (pDataDefs->arrDataDescriptions[i].type == Descriptor_RigidBody) {
      ...
      // skip for now since id could clash with non-skeleton RigidBody ids in
      // our RigidBody lookup table
      /*
      if (insertResult.second == true)
      {
          ...
          {
              // Note:
              // In the DataCallback packet (sFrameOfMocapData) skeleton bones
      (rigid bodies) ids are of the form:
              //   parent skeleton ID   : high word (upper 16 bits of int)
              //   rigid body id        : low word  (lower 16 bits of int)
              //
              // In DataDescriptions packet (sDataDescriptions) they are not, so
      apply the data id format here
              // for correct lookup during data callback
              std::pair<std::map<int, std::string>::iterator, bool>
      insertBoneResult; sRigidBodyDescription rb = pSK->RigidBodies[j]; int id =
      (rb.parentID << 16) | rb.ID; std::string skeletonBoneName =
      string(pSK->szName) + (":") + string(rb.szName) + string(pSK->szName);
              insertBoneResult = g_AssetIDtoAssetName.insert(id,
      skeletonBoneName);
          }
      }
      */
...
void OutputFrameQueueToConsole() {
  ...
  for (MocapFrameWrapper f : displayQueue) {
    ...
    //   a) The reception of a complete group of 2D frames from the camera
    //   system (CameraDataReceivedTimestamp)
    // and
    //   b) The time immediately prior to the NatNet frame being transmitted
    //   over the network (TransmitTimestamp)
    //
    // This figure may appear slightly higher than the "software latency"
    // reported in the Motive user interface, because it additionally includes
    // the time spent preparing to stream the data via NatNet.
    ...
    const double softwareLatencyMillisec =
        ...
        static_cast<double>(g_serverDescription.HighResClockFrequency);
    ...
    // camera systems support system latency measurement. If it's unavailable
    // (for example, with USB camera systems, or during playback), this field
    // will be zero.
    ...
    if (bSystemLatencyAvailable) {
      ...
      // Transit latency is defined as the span of time between Motive
      // transmitting the frame of data, and its reception by the client (now).
      // The SecondsSinceHostTimestamp method relies on NatNetClient's internal
      // clock synchronization with the server using Cristian's algorithm.
    ...
    for (int i = 0; i < data->nAssets; i++) {
      ...
      for (int j = 0; j < asset.nMarkers; j++) {
        ...
        printf("Marker [AssetID=%d, MarkerID=%d] [size=%3.2f] "
    ...
    for (i = 0; i < data->nLabeledMarkers; i++) {
      ...
      printf("%s Marker [ModelID=%d, MarkerID=%d] [size=%3.2f] "
             "[pos=%3.2f,%3.2f,%3.2f] [residual(mm)=%.4f]\n",
    ...
    for (int iPlate = 0; iPlate < data->nForcePlates; iPlate++) {
      ...
      for (int iChannel = 0; iChannel < data->ForcePlates[iPlate].nChannels;
        ...
        for (int iSample = 0;
             iSample < data->ForcePlates[iPlate].ChannelData[iChannel].nFrames;
             iSample++)
          printf(
              "%3.2f\t",
              data->ForcePlates[iPlate].ChannelData[iChannel].Values[iSample]);
        printf("\n");
      }
    }
    ...
    for (int iDevice = 0; iDevice < data->nDevices; iDevice++) {
      ...
      for (int iChannel = 0; iChannel < data->Devices[iDevice].nChannels;
        ...
        for (int iSample = 0;
             ...
             iSample++)
          printf("%3.2f\t",
                 data->Devices[iDevice].ChannelData[iChannel].Values[iSample]);
        printf("\n");
      }
    }
  }
...
void WriteHeader(FILE* fp, sDataDescriptions* pBodyDefs) {
  ...
  sMarkerSetDescription* pMS =
      pBodyDefs->arrDataDescriptions[0].Data.MarkerSetDescription;
...
void WriteFrame(FILE* fp, sFrameOfMocapData* data) {
  ...
  for (int i = 0; i < data->MocapData->nMarkers; i++) {
    fprintf(fp, "\t%.5f\t%.5f\t%.5f", data->MocapData->Markers[i][0],
            data->MocapData->Markers[i][1], data->MocapData->Markers[i][2]);
  }
  fprintf(fp, "\n");
}
...
int SetGetProperty(char* szSetGetCommand) {
  ...
  if (iResult == ErrorCode_OK) {
    // Note : we sleep here to give Motive a chance to process the above
    // SetProperty on a safe thread.  Otherwise Motive may return the previous
    // value
