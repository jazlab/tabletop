...
def test_hash2(
    test_name, test_hash_str, test_object, generator_string, run_test
):
    ret_value = K_FAIL
    out_str = "FAIL"
    out_str2 = ""
...
def stringify_timecode(timecode, timecode_sub):
    """prints out timecode"""
    hour, minute, second, frame, subframe = decode_timecode(
        timecode, timecode_sub
    )  # type: ignore  # noqa E501
    timecode_string = (
        f"{hour:02}:{minute:02}:{second:02}:{frame:02}:{subframe:02}"  # type: ignore  # noqa E501
    )
...
class Skeleton:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        for rb_num in range(rigid_body_count):
            ...
            out_str += self.rigid_body_list[rb_num].get_as_string(
                tab_str, level + 2
            )
        return out_str
...
class AssetData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        for asset_num in range(asset_count):
            ...
            out_str += self.asset_list[asset_num].get_as_string(
                tab_str, level + 2
            )
        return out_str
...
class ForcePlateChannelData:
    ...
    def get_as_string(self, tab_str, level, channel_num=-1):
        ...
        if fc_show < frame_count:
            out_str += " - Showing %3.1d of %3.1d frames" % (
                fc_show,
                frame_count,
            )
        out_str += "\n"
...
class ForcePlateData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        for i in range(force_plate_count):
            ...
            out_str += self.force_plate_list[i].get_as_string(
                tab_str, level + 2
            )
...
class FrameSuffixData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        if not self.timecode == -1 and not self.timecode_sub == -1:
            self.timecode = stringify_timecode(
                self.timecode, self.timecode_sub
            )
...
class MoCapData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        if self.labeled_marker_data is not None:
            out_str += self.labeled_marker_data.get_as_string(
                tab_str, level + 1
            )
        else:
...
def generate_rigid_body(body_num=0, frame_num=0):
    ...
    rigid_body.add_rigid_body_marker(
        generate_rigid_body_marker_srand(0, frame_num)
    )
    rigid_body.add_rigid_body_marker(
        generate_rigid_body_marker_srand(1, frame_num)
    )
...
def generate_force_plate(frame_num=0, fp_num=0, num_channels=1):
    ...
    # add channel_data
    for i in range(num_channels):
        force_plate.add_channel_data(
            generate_fp_channel_data(frame_num, fp_num, i, 10)
        )
    return force_plate
...
def generate_device(frame_num=0, device_num=0):
    ...
    device.add_channel_data(
        generate_device_channel_data(frame_num, device_num, 1, 4)
    )
    device.add_channel_data(
        generate_device_channel_data(frame_num, device_num, 3, 2)
    )
    device.add_channel_data(
        generate_device_channel_data(frame_num, device_num, 7, 6)
    )
    return device
