...
unsigned char gCommandResponseString[4096];
...
void* DataListenThread(void* dummy) {
  ...
  while (true) {
    ...
    // ourselves)
    int nDataBytesReceived =
        recvfrom(gDataSocket, szData, nDataBytes, 0,
                 (struct sockaddr*) &TheirAddress, &addr_len);
    ...
    if (nDataBytesReceived > 0) {
      ...
      if (wsaLastError == 10040) {
        ...
        if (nBytesTotal <= MAX_PACKETSIZE) {
          ...
          newSize = std::min(newSize, (int) MAX_PACKETSIZE);
...
SOCKET CreateCommandSocket(unsigned long IP_Address, unsigned short uPort,
  ...
  if (useMulticast) {
    ...
    int timeout = 2000;
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (const void*) &timeout,
               sizeof timeout);
  }
  retval =
  ...
  // Unicast case
    ...
    // standard ethernet MTU (~1500 bytes). On Linux, we use IP_MTU_DISCOVER
    // with IP_PMTUDISC_DONT to allow fragmentation
    int optval2 = IP_PMTUDISC_DONT;
    socklen_t optlen = sizeof(int);
    int iRet = setsockopt(sockfd, IPPROTO_IP, IP_MTU_DISCOVER, (char*) &optval2,
                          sizeof(optval2));
    if (iRet == -1) {
...
int main(int argc, char* argv[]) {
  ...
  commandListenThread = std::thread(CommandListenThread, nullptr);
  printf("[PacketClient Main] CommandListenThread started\n");

  // startup our "Data Listener" thread
  dataListenThread = std::thread(DataListenThread, nullptr);
  ...
  // Wait for threads to finish (they run indefinitely, so this won't normally
  // be reached)
  if (commandListenThread.joinable()) {
    commandListenThread.detach();
  }
  if (dataListenThread.joinable()) {
    dataListenThread.detach();
  }
...
bool IPAddress_StringToAddr(char* szNameOrAddress, struct in_addr* Address) {
  ...
  // ANSI host name
  if ((retVal = getnameinfo((struct sockaddr*) &saGNI, sizeof(sockaddr),
