 ...
 *     [ServerIP]			IP address of server ( defaults to local
 * machine) [LocalIP]			IP address of client ( defaults to local
 * machine)
 */
...
struct sConnectionOptions {
  ...
  sConnectionOptions()
      : subscribedDataOnly(false), BitstreamVersion{0, 0, 0, 0} {
  }
#endif
};
...
char* UnpackPacketHeader(char* ptr, int& messageID, int& nBytes,
                         int& nBytesTotal, unsigned int level);
char* UnpackDataSize(char* ptr, int major, int minor, int& nBytes,
                     unsigned int level, bool skip = false);
...
char* UnpackMarkersetDescription(char* ptr, char* targetPtr, int major,
                                 int minor, unsigned int level);
char* UnpackRigidBodyDescription(char* ptr, char* targetPtr, int major,
                                 int minor, unsigned int level);
...
DWORD WINAPI CommandListenThread(void* dummy) {
  ...
  if (PacketIn && PacketOut) {
    ...
    while (true) {
      ...
      Command=%d, nDataBytes=%d", TheirAddress.sin_addr.S_un.S_un_b.s_b1,
...
DWORD WINAPI DataListenThread(void* dummy) {
  ...
  if (szData) {
    ...
    printf("[PacketClient DLTh] DataListenThread Start FAILURE memory "
           "allocation\n");
  }
  ...
  while (true) {
    ...
    // ourselves)
    int nDataBytesReceived = recvfrom(gDataSocket, szData, nDataBytes, 0,
    ...
    if (nDataBytesReceived > 0) {
      ...
      if (wsaLastError == 10040) {
        ...
        if (nBytesTotal <= MAX_PACKETSIZE) {
          ...
          if (szDataNew) {
            printf("[PacketClient DLTh] Resizing data buffer from %d bytes to "
                   "%d bytes",
                   nDataBytes, newSize);
            if (szData) {
              delete[] szData;
            ...
            printf("PacketClient DLTh] Data buffer size failure have %d bytes "
                   "but need %d bytes",
                   nDataBytes, nBytesTotal);
          }
        } else {
          printf("PacketClient DLTh] Data buffer size failure have %d bytes "
                 "but need %d bytes",
...
SOCKET CreateCommandSocket(unsigned long IP_Address, unsigned short uPort,
  ...
  retval =
      getsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, (char*) &optval, &optval_size);
  ...
  if (optval != OPTVAL_REQUEST_SIZE) {
    // err - actual size...
    printf("[PacketClient Main] gCommandSocket Receive Buffer size = %d "
           "requested %d\n",
  ...
  // Unicast case
    ...
    // set a read timeout to allow for sending keep_alive message for unicast
    // clients
    int timeout = 2000;
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (const char*) &timeout,
               sizeof timeout);

    // allow multiple clients on same machine to use multicast group
    // address/port
    int value = 1;
    int retval = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char*) &value,
                            sizeof(value));
    if (retval == -1) {
      printf(
    ...
    retval = getsockopt(sockfd, SOL_SOCKET, SO_SNDBUF,
                        (char*) &defaultBufferSize, &optval_size);
    retval = setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (char*) &bufSize,
                        sizeof(bufSize));
    if (retval == -1) {
    ...
    int confirmValue = 0;
    getsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (char*) &confirmValue,
               &optval_size);
    if (confirmValue != bufSize) {
      // not fatal, but notify user requested size is not valid
      printf("[PacketClient Main] gCommandSocket buffer smaller than expected "
             "%d instead of %d\n",
             confirmValue, bufSize);
    }

    // Set "Don't Fragment" bit in IP header to false (0).
    // note : we want fragmentation support since our packets are over the
    // standard ethernet MTU (~1500 bytes).
    int optval2;
    socklen_t optlen = sizeof(int);
    int iRet = getsockopt(sockfd, IPPROTO_IP, IP_DONTFRAGMENT, (char*) &optval2,
                          &optlen);
    optval2 = 0;
    iRet = setsockopt(sockfd, IPPROTO_IP, IP_DONTFRAGMENT, (char*) &optval2,
                      sizeof(optval2));
    if (iRet == -1) {
    ...
    iRet = getsockopt(sockfd, IPPROTO_IP, IP_DONTFRAGMENT, (char*) &optval2,
                      &optlen);
  }
...
SOCKET CreateDataSocket(unsigned long socketIPAddress,
  ...
  retval = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char*) &value,
                      sizeof(value));
  if (retval == SOCKET_ERROR) {
 ...
 * \param useMulticast
 */
void PrintConfiguration(const char* szMyIPAddress,
                        const char* szServerIPAddress, bool useMulticast) {
  ...
  printf("    %s Version  %d %d %d %d\n", gServerName, gServerVersion[0],
         gServerVersion[1], gServerVersion[2], gServerVersion[3]);
  printf("    NatNetVersion  %d %d %d %d\n", gNatNetVersionServer[0],
         gNatNetVersionServer[1], gNatNetVersionServer[2],
         gNatNetVersionServer[3]);
  printf("  NatNet Bitstream Requested\n");
  printf("    NatNetVersion  %d %d %d %d\n", gNatNetVersion[0],
         gNatNetVersion[1], gNatNetVersion[2], gNatNetVersion[3]);
...
void PrintCommands(bool canChangeBitstream) {
  printf(
      ...
      "  v Get the version of the currently streaming bitstream (Unicast "
      "Only)\n"
...
int main(int argc, char* argv[]) {
  ...
  int commandPort = 0;
  gCommandSocket = CreateCommandSocket(parsedArgs.myAddress.S_un.S_addr,
                                       commandPort, optval, gUseMulticast);
  if (gCommandSocket == -1) {
    // error
    printf("[PacketClient Main] gCommandSocket create failure (error: %s)\n",
           GetWSAErrorString(WSAGetLastError()).c_str());
  ...
  int dataPort = 0;
  gDataSocket =
      CreateDataSocket(parsedArgs.myAddress.S_un.S_addr, dataPort, optval,
                       parsedArgs.useMulticast,
                       parsedArgs.multiCastAddress.S_un.S_addr, PORT_DATA);
  if (gDataSocket == -1) {
  ...
  commandListenThreadHandle =
      CreateThread(&commandListenSecurityAttribs, 0, CommandListenThread,
                   nullptr, 0, &commandListenThreadID);
  printf("[PacketClient Main] CommandListenThread started\n");

  // startup our "Data Listener" thread
  dataListenThread_Handle =
      CreateThread(&dataListenThreadSecurityAttribs, 0, DataListenThread,
                   nullptr, 0, &dataListenThread_ID);
  ...
  memcpy(&PacketOut->Data.cData[(int) sizeof(sSender)], &connectOptions,
         sizeof(connectOptions));

  int nTries = 3;
  int iRet = SOCKET_ERROR;
  while (nTries--) {
    iRet = sendto(gCommandSocket, (char*) PacketOut, 4 + PacketOut->nDataBytes,
                  0, (sockaddr*) &gHostAddr, sizeof(gHostAddr));
    if (iRet != SOCKET_ERROR)
      break;
  }
  ...
  PrintConfiguration(parsedArgs.szMyIPAddress, parsedArgs.szServerIPAddress,
                     parsedArgs.useMulticast);
  ...
  while (!bExit) {
    ...
    case 'o': {
      ...
      int returnCode;
      std::vector<std::string> commandVec{"TimelineStop",
...
bool IPAddress_StringToAddr(char* szNameOrAddress, struct in_addr* Address) {
  ...
  // getnameinfo in WS2tcpip is protocol independent and resolves address to
  // ANSI host name
  if ((retVal = getnameinfo((SOCKADDR*) &saGNI, sizeof(sockaddr), hostName,
                            MAX_NAMELENGTH, servInfo, MAX_NAMELENGTH,
                            NI_NUMERICSERV)) != 0) {
...
/**
 * \brief Funtion that assigns a time code values to 5 variables passed as
 * arguments Requires an integer from the packet as the timecode and
 * timecodeSubframe
...
/**
 * \brief Receives pointer to byes of a data description and decodes based on
 * major/minor version
 * \param inptr - input
...
char* UnpackDescription(char* inptr, int nBytes, int major, int minor,
  ...
  for (int i = 0; i < nDatasets; i++) {
    ...
    case 3: // force plate
      ...
      ptr =
          UnpackForcePlateDescription(ptr, targetPtr, major, minor, level + 4);
      break;
    case 4: // device
...
char* UnpackMarkersetDescription(char* ptr, char* targetPtr, int major,
  ...
  for (int j = 0; j < nMarkers; j++) {
    ...
    if (ptr > targetPtr) {
      printf("%sUnpackMarkersetDescription: UNPACK ERROR DETECTED: STOPPING "
             "DECODE\n",
             outTabStr.c_str());
      return ptr;
    }
 ...
 * \return - pointer after decoded object
 */
char* UnpackRigidBodyDescription(char* inptr, char* targetPtr, int major,
                                 int minor, unsigned int level) {
  std::string outTabStr = GetTabString(kTabStr, level);
  ...
  if (ptr > targetPtr) {
    printf("%sUnpackRigidBodyDescription: UNPACK ERROR DETECTED: STOPPING "
           "DECODE\n",
           outTabStr.c_str());
    return ptr;
  }
  ...
  if (major > 4 || (major == 4 && minor >= 2) || major == 0) {
    // Rotation Offsets
    float qxoffset = 0;
    memcpy(&qxoffset, ptr, 4);
    ptr += 4;
    float qyoffset = 0;
    memcpy(&qyoffset, ptr, 4);
    ptr += 4;
    float qzoffset = 0;
    memcpy(&qzoffset, ptr, 4);
    ptr += 4;
    float qwoffset = 0;
    memcpy(&qwoffset, ptr, 4);
    ptr += 4;
  ...
  if ((major >= 3) || (major == 0)) {
    ...
    if (nMarkers > 16000) {
      ...
      printf("%sUnpackRigidBodyDescription: UNPACK ERROR DETECTED: STOPPING "
             ...
             outTabStr.c_str(), nBytesProcessed);
    ...
    if (nMarkers > 0) {
      ...
      for (int markerIdx = 0; markerIdx < nMarkers; ++markerIdx) {
        ...
        printf("%s%3.1d Marker Label: %3.1d Position: [%3.2f %3.2f %3.2f] %s\n",
               outTabStr.c_str(), markerIdx, label, xpos, ypos, zpos,
               szMarkerName);
        if (ptr3 > targetPtr) {
          printf("%sUnpackRigidBodyDescription: UNPACK ERROR DETECTED: "
                 "STOPPING DECODE\n",
                 outTabStr.c_str());
          return ptr3;
        }
...
char* UnpackSkeletonDescription(char* ptr, char* targetPtr, int major,
  ...
  if (ptr > targetPtr) {
    printf(
        "%sUnpackSkeletonDescription: UNPACK ERROR DETECTED: STOPPING DECODE\n",
        outTabStr.c_str());
    return ptr;
  }
  ...
  for (int i = 0; i < nRigidBodies; i++) {
    ...
    if (ptr > targetPtr) {
      printf("%sUnpackSkeletonDescription: UNPACK ERROR DETECTED: STOPPING "
             "DECODE\n",
             outTabStr.c_str());
      return ptr;
    }
...
char* UnpackDeviceDescription(char* ptr, char* targetPtr, int major, int minor,
  ...
  if ((major >= 3) || (major == 0)) {
    ...
    if (ptr > targetPtr) {
      printf(
          "%sUnpackDeviceDescription: UNPACK ERROR DETECTED: STOPPING DECODE\n",
          outTabStr.c_str());
      return ptr;
    }
    ...
    for (int chNum = 0; chNum < nChannels; ++chNum) {
      ...
      printf("%s  Channel %d Name  : %s\n", outTabStr.c_str(), chNum,
             szChannelName);
      if (ptr > targetPtr) {
        printf("%sUnpackDeviceDescription: UNPACK ERROR DETECTED: STOPPING "
               "DECODE\n",
               outTabStr.c_str());
        return ptr;
      }
...
char* UnpackCameraDescription(char* ptr, char* targetPtr, int major, int minor,
  ...
  printf("%s  Orientation: [%3.2f, %3.2f, %3.2f, %3.2f]\n", outTabStr.c_str(),
         cameraOriQuat[0], cameraOriQuat[1], cameraOriQuat[2],
         cameraOriQuat[3]);
...
char* UnpackAssetDescription(char* ptr, char* targetPtr, int major, int minor,
  ...
  for (int i = 0; i < nRigidBodies; i++) {
    ...
    if (ptr > targetPtr) {
      printf(
          "%sUnpackAssetDescription: UNPACK ERROR DETECTED: STOPPING DECODE\n",
          outTabStr.c_str());
      return ptr;
    }
  }
...
/**
 * \brief legacy 'other' unlabeled marker and print contents (will be
 * deprecated)
 * \param ptr - input data stream pointer
 * \param major - NatNet major version
 * \param minor - NatNet minor version
    ...
    if (major < 3) {
      ...
      if (major >= 2) {
        ...
        for (int k = 0; k < nRigidMarkers; k++) {
          printf(
              "%s  Marker %d: id=%d  size=%3.1f  pos=[%3.2f, %3.2f, %3.2f]\n",
              outTabStr.c_str(), k, markerIDs[k], markerSizes[k],
      ...
      // Print marker positions for all rigid bodies
        ...
        for (int k = 0; k < nRigidMarkers; k++) {
          ...
          printf("%s  Marker %d: pos : [%3.2f, %3.2f, %3.2f]\n",
                 outTabStr.c_str(), k, markerData[k3], markerData[k3 + 1],
                 markerData[k3 + 2]);
        }
      }
...
char* UnpackAssetMarkerData(char* ptr, int major, int minor,
  ...
  printf("%s  Marker %d\tpos : [%3.2f, %3.2f, "
         "%3.2f]\tsize=%3.2f\terr=%3.2f\tparams=%d\n",
         outTabStr.c_str(), ID, x, y, z, size, residual, params);
...
char* UnpackLabeledMarkerData(char* ptr, int major, int minor,
  ...
  if (((major == 2) && (minor >= 3)) || (major > 2)) {
    ...
    for (int j = 0; j < nLabeledMarkers; j++) {
      ...
      bool bHasModel =
          false; // marker has an associated asset in the data stream
      bool bUnlabeled =
          false; // marker is 'unlabeled', but has a point cloud ID
...
char* UnpackFrameSuffixData(char* ptr, int major, int minor,
  ...
  gBitstreamVersionChanged =
      (params & 0x04) != 0; // 0x04 Bitstream syntax version has changed
...
char* Unpack(char* pData, unsigned int level) {
  ...
  case NAT_FRAMEOFDATA: {
    ...
    */
    ...
    uint16_t params;
    char* ptrToParams =
        ptr + (nBytes - 6); // 4 bytes for terminating 0 + 2 bytes for params
    ...
    gBitstreamVersionChanged =
        (params & 0x08) != 0; // 0x04 Bitstream syntax version has changed
