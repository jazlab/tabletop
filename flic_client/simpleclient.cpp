...
static const char* CreateConnectionChannelErrorStrings[] = {
    "NoError", "MaxPendingConnectionsReached"};

static const char* ConnectionStatusStrings[] = {"Disconnected", "Connected",
                                                "Ready"};

static const char* DisconnectReasonStrings[] = {
    "Unspecified", "ConnectionEstablishmentFailed", "TimedOut",
    "BondingKeysMismatch"};

static const char* RemovedReasonStrings[] = {"RemovedByThisClient",
...
static const char* BdAddrTypeStrings[] = {"PublicBdAddrType",
                                          "RandomBdAddrType"};

static const char* LatencyModeStrings[] = {"NormalLatency", "LowLatency",
                                           "HighLatency"};
...
static const char* BluetoothControllerStateStrings[] = {"Detached", "Resetting",
                                                        "Attached"};
...
struct Bdaddr {
  ...
  bool operator==(const Bdaddr& o) const {
...
static void print_help() {
  static const char help_text[] =
      "Available commands:\n"
      "getInfo - get various info about the server state and previously "
      "verified buttons\n"
      ...
      "connect xx:xx:xx:xx:xx:xx id - first parameter is the bluetooth address "
      "of the button, second is an integer identifier you set to identify this "
      "connection\n"
      "disconnect id - disconnect or abort pending connection\n"
      "changeModeParameters id latency_mode auto_disconnect_time - change "
      "latency mode (NormalLatency/LowLatency/HighLatency) and auto disconnect "
      "time for this connection\n"
      "forceDisconnect xx:xx:xx:xx:xx:xx - disconnect this button, even if "
      "other client program are connected\n"
      "getButtonInfo xx:xx:xx:xx:xx:xx - get button info for a verified "
      "button\n"
      "createBatteryStatusListener xx:xx:xx:xx:xx:xx id - first parameter is "
      "the bluetooth address of the button, second is an integer you set to "
      "identify this listener\n"
...
int main(int argc, char* argv[]) {
  // Disable buffering for stdin to be able to select on both the server socket
  // and stdin
  ...
  while (1) {
    ...
    while (true) {
      int select_res =
          select(max(STDIN_FILENO, sockfd) + 1, &fdread, NULL, NULL, NULL);
      if (select_res < 0) {
        if (errno == EINTR) {
          continue;
        } else {
    ...
    if (ioctl(sockfd, FIONREAD, &bytes_available) >= 0) {
      ...
      case EVT_ADVERTISEMENT_PACKET_OPCODE: {
        ...
        printf("ADV: %s %s %d %s %s%s%s\n",
               ...
               string(evt->name, (size_t) evt->name_length).c_str(), evt->rssi,
               (evt->is_private ? "private" : "public"),
               (evt->already_verified ? "verified" : "unverified"),
               (evt->already_connected_to_this_device
                    ? " already connected to this device"
                    : ""),
               (evt->already_connected_to_other_device
                    ? " already connected to other device"
                    : ""));
        break;
      }
      case EVT_CREATE_CONNECTION_CHANNEL_RESPONSE_OPCODE: {
        EvtCreateConnectionChannelResponse* evt =
            (EvtCreateConnectionChannelResponse*) pkt;
        printf("Create conn: %d %s %s\n", evt->base.conn_id,
               CreateConnectionChannelErrorStrings[evt->error],
               ConnectionStatusStrings[evt->connection_status]);
        break;
      }
      case EVT_CONNECTION_STATUS_CHANGED_OPCODE: {
        EvtConnectionStatusChanged* evt = (EvtConnectionStatusChanged*) pkt;
        printf("Connection status changed: %d %s", evt->base.conn_id,
               ConnectionStatusStrings[evt->connection_status]);
      ...
      case EVT_CONNECTION_CHANNEL_REMOVED_OPCODE: {
        ...
        printf("Connection removed: %d %s\n", evt->base.conn_id,
               RemovedReasonStrings[evt->removed_reason]);
        break;
      }
      ...
      case EVT_BUTTON_SINGLE_OR_DOUBLE_CLICK_OR_HOLD_OPCODE: {
        ...
        static const char* types[] = {"Button up/down", "Button click/hold",
                                      "Button single/double click",
                                      "Button single/double click/hold"};
        printf("%s: %d, %s, %s, %d seconds ago\n",
               types[readbuf[0] - EVT_BUTTON_UP_OR_DOWN_OPCODE],
               evt->base.conn_id, ClickTypeStrings[evt->click_type],
               (evt->was_queued ? "queued" : "not queued"), evt->time_diff);
        break;
      }
      ...
      case EVT_GET_INFO_RESPONSE_OPCODE: {
        ...
        printf("Got info: %s, %s (%s), max pending connections: %d, max conns: "
               "%d, current pending conns: %d, currently no space: %c\n",
        ...
        puts(evt->nb_verified_buttons > 0 ? "Verified buttons:"
      ...
      case EVT_BLUETOOTH_CONTROLLER_STATE_CHANGE_OPCODE: {
        EvtBluetoothControllerStateChange* evt =
            (EvtBluetoothControllerStateChange*) pkt;
      ...
      case EVT_GET_BUTTON_INFO_RESPONSE_OPCODE: {
        ...
        printf("Button info response: %s %s %s %s %d %d\n",
               ...
               string(evt->serial_number, (size_t) evt->serial_number_length)
                   .c_str(),
               evt->flic_version, evt->firmware_version);
        break;
      }
      case EVT_SCAN_WIZARD_FOUND_PRIVATE_BUTTON_OPCODE: {
        printf("Found private button. Please hold down it for 7 seconds to "
               "make it public.\n");
        break;
      }
      case EVT_SCAN_WIZARD_FOUND_PUBLIC_BUTTON_OPCODE: {
        EvtScanWizardFoundPublicButton* evt =
            (EvtScanWizardFoundPublicButton*) pkt;
      ...
      case EVT_BATTERY_STATUS_OPCODE: {
        ...
        printf("Battery status report for id %d, percentage: %d%%, timestamp: "
               "%s\n",
               evt->listener_id, evt->battery_percentage,
               ctime((time_t*) &evt->timestamp));
        break;
      }
      case EVT_BUTTON_DELETED_OPCODE: {
        ...
        printf("Button %s deleted %s\n",
               Bdaddr(evt->bd_addr).to_string().c_str(),
               evt->deleted_by_this_client ? "by this client"
                                           : "not by this client");
        break;
      }
      }
