...
def got_button(bd_addr):
    ...
    cc.on_button_up_or_down = (
        lambda channel, click_type, was_queued, time_diff: print(
            channel.bd_addr + " " + str(click_type)
        )
    )
    cc.on_connection_status_changed = (
        lambda channel, connection_status, disconnect_reason: print(
            channel.bd_addr
            + " "
            + str(connection_status)
            + (
                " " + str(disconnect_reason)
                if connection_status == fliclib.ConnectionStatus.Disconnected
                else ""
            )
        )
    )
...
class T(threading.Thread):
    def run(self):
        ...
        scanner.on_advertisement_packet = (
            ...
            already_connected_to_other_device: print(
                bd_addr
                + " "
                + name
                + " "
                + ("Private" if is_private else "Public")
                + (" already verified" if already_verified else "")
            )
        )
